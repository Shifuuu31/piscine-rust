...
mod tests {
    ...
    fn test_correct_answer_on_first_try() {
        ...
        stdin.write_all(ANSWER.as_bytes()).expect("Failed to write to stdin");
    ...
    fn test_more_than_one_trial_to_get_the_right_answer() {
        ...
        for _ in 0..n_incorrect_tries {
            stdin.write_all("incorrect answer\n".as_bytes()).expect("Failed to write to stdin");
        }
        stdin.write_all(ANSWER.as_bytes()).expect("Failed to write to stdin");
        ...
        assert_eq!(
            String::from_utf8(output.stdout).expect("Invalid output"),
            format!("{}Number of trials: {}\n", expected_output, n_incorrect_tries + 1)
        );
    }
}
